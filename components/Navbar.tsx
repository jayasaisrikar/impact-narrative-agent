    ...
    <header className="sticky top-0 z-30 bg-white/80 shadow-xl shadow-blue-500/10 backdrop-blur border-b border-white/30">
      <div className="container mx-auto flex items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          >
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-purple-500 text-white shadow-lg">
              INA
            </span>
      ...
      {open && (
        <div id="mobile-menu" className="space-y-3 border-t border-white/30 bg-white/90 px-4 pb-4 pt-2 text-sm text-text-secondary shadow-lg md:hidden">
          <button
            onClick={() => handleNav('dashboard')}
            className={`block w-full rounded-full px-4 py-2 text-left transition ${currentTab === 'dashboard' ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-100 hover:text-text-primary'}`}
          >
            Dashboard
          </button>
          <button
            onClick={() => handleNav('chatbot')}
            className={`block w-full rounded-full px-4 py-2 text-left transition ${currentTab === 'chatbot' ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-100 hover:text-text-primary'}`}
          >
            Chatbot
          </button>
          {onReturnToLanding && (
